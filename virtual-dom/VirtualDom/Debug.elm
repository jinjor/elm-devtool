...
wrapUpdate
  ...
  -> Task Never ()
  -> (msg -> model -> Task Never ())
  -> Msg msg
  -> Model model msg
  -> (Model model msg, Cmd (Msg msg))
wrapUpdate userUpdate scrollTask sendToBackendTask msg model =
  case msg of
    NoOp ->
      model ! []

    UserMsg userMsg ->
      updateUserMsg userUpdate scrollTask sendToBackendTask userMsg model
    ...
    Up ->
      ...
      in
        if index > 0 then
          wrapUpdate userUpdate scrollTask sendToBackendTask (Jump (index - 1)) model
        else
          model ! []
      ...
      case model.state of
        ...
        Paused index _ userModel ->
          if index == History.size model.history - 1 then
            wrapUpdate userUpdate scrollTask sendToBackendTask Resume model
          else
            wrapUpdate userUpdate scrollTask sendToBackendTask (Jump (index + 1)) model
...
updateUserMsg
  ...
  -> Model model msg
  -> (Model model msg, Cmd (Msg msg))
updateUserMsg userUpdate scrollTask sendToBackendTask userMsg ({ history, state, expando } as model) =
  let
    userModel =
    ...
    case state of
      Running _ ->
        { model
          ...
          ! [ commands
            , runIf True scrollTask
            , runIf True (sendToBackendTask userMsg newUserModel)
            ]
